...
const navigationMenuLogoTriggerStyle = cva(
    "group inline-flex h-9 w-max items-center justify-center rounded-md bg-background pr-4 py-1 text-gray-300 font-medium "
)

const navigationMenuRightTriggerStyle = cva(
    "group inline-flex h-9 w-max items-center justify-center rounded-md bg-background px-4 py-2 " +
    "text-base font-medium transition-colors hover:bg-black hover:text-white/50 focus:bg-accent " +
    "focus:text-accent-foreground focus:outline-none disabled:pointer-events-none disabled:opacity-50 " +
    "data-[active]:bg-black/50 data-[state=open]:bg-black/50"
)
...
>(({ className, ...props }, ref) => (
  <NavigationMenuPrimitive.Content
    ref={ref}
    className={cn("w-full data-[motion^=from-]:animate-in data-[motion^=to-]:animate-out data-[motion^=from-]:fade-in " +
    ...
    <NavigationMenuPrimitive.Viewport
      className={cn(
        "origin-top-center relative mt-1.5 h-[var(--radix-navigation-menu-viewport-height)] w-full overflow-hidden " +
          "rounded-md border bg-popover text-popover-foreground shadow data-[state=open]:animate-in data-[state=closed]:animate-out " +
...
export {
    navigationMenuRightTriggerStyle,
