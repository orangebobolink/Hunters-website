...
const PermissionInfoDialog = ({permission, isOpen, setIsOpen}: IProps) => {
    ...
    const handlerReceived = async () => {
        ...
        permission.receivedDate = new Date()
        const response = await FeedingService.update(permission)
        ...
        <Dialog open={isOpen} onOpenChange={()=>setIsOpen(false)}>
            <DialogContent>
                <div className="p-4 shadow-md rounded-lg">
                    <p className="font-bold">Номер: <span className="font-normal">{permission.number}</span></p>
                    <p className="font-bold">Дата подкормки: <span className="font-normal">{format(permission.fromDate, "MM/dd/yyyy")}</span></p>
                ...
                {
                    (roles.includes("Ranger") && permission.status == Status[Status.Given]) &&
