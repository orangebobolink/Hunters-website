            ...
            <DialogContent>
                <div className='p-4 shadow-md rounded-lg'>
                    <p className='font-bold'>
                        Номер:{' '}
                        <span className='font-normal'>{feeding.number}</span>
                    </p>
                    <p className='font-bold'>
                        Дата подкормки:{' '}
                        <span className='font-normal'>
                            {format(feeding.feedingDate, 'MM/dd/yyyy')}
                        </span>
                    </p>
                    <p className='font-bold'>
                        Составитель:{' '}
                        <span className='font-normal'>
                            {UserService.getFullName(issued)}
                        </span>
                    </p>
                    <p className='font-bold'>
                        Выданно егерю:{' '}
                        <span className='font-normal'>
                            {UserService.getFullName(recieved)}
                        </span>
                    </p>
                    <p className='font-bold'>
                        Дата получения:{' '}
                        <span className='font-normal'>
                            {feeding.receivedDate
                                ? format(feeding.receivedDate, 'MM/dd/yyyy')
                                : 'Not Received'}
                        </span>
                    </p>
                    <p className='font-bold'>
                    ...
                    <p className='font-bold'>
                        Локация:{' '}
                        <span className='font-normal'>{feeding.land.name}</span>
                    </p>
                    ...
                    <ul className='list-disc pl-5'>
                        {feeding.products.map((product, index) => (
                            <li key={index} className='font-normal'>
                                {product.product == 'Oats' ? 'Овес' : 'Пшеница'}
                ...
                {roles.includes('Ranger') &&
                    feeding.status == Status[Status.Given] && (
                        <Button onClick={handlerReceived}>Приянть</Button>
                    )}
                {roles.includes('Ranger') &&
                    feeding.status == Status[Status.Recived] && (
                        <Button onClick={handlerComplited}>
