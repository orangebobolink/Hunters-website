            ...
            <DialogContent>
                <div className='p-4 shadow-md rounded-lg'>
                    <p className='font-bold'>
                        Номер:{' '}
                        <span className='font-normal'>{trip.number}</span>
                    </p>
                    <p className='font-bold'>
                        Дата подкормки:{' '}
                        <span className='font-normal'>
                            {format(trip.permission!.fromDate, 'MM/dd/yyyy')}
                        </span>
                    </p>
                    <p className='font-bold'>
                        Дата подкормки:{' '}
                        <span className='font-normal'>
                            {format(trip.permission!.toDate, 'MM/dd/yyyy')}
                        </span>
                    </p>
                    <p className='font-bold'>
                        Составитель:{' '}
                        <span className='font-normal'>
                            {UserService.getFullName(trip.permission!.issued!)}
                        </span>
                    </p>
                    <p className='font-bold'>
                        Выданно егерю:{' '}
                    ...
                    <p className='font-bold'>
                        Дата охоты:
                        <span> </span>
                        {format(trip.eventDate!, 'MM/dd/yyyy') ===
                        '01/01/0001' ? (
                ...
                {roles.includes('Ranger') &&
                    (format(trip.eventDate!, 'MM/dd/yyyy') === '01/01/0001' ? (
                        ...
                        trip.status !== Status.Completed.toString() && (
                            <Button onClick={handlerUpdateStatus}>
                                Пометить как проведенное
                            </Button>
                        )
                    ))}
            </DialogContent>
