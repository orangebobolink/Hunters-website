...
const RaidTable = ({ raids }: IProps) => {
    ...
    const content = (raid: Raid) => (
        <TableRow key={raid.id}>
            <TableCell>{raid.land.name}</TableCell>
            <TableCell>{format(raid.exitTime, 'dd.MM.yyyy')}</TableCell>
            <TableCell>{format(raid.returnedTime, 'dd.MM.yyyy')}</TableCell>
            <TableCell>
                {EnumService.statysTranslate(raid.status.toString())}
            </TableCell>
        </TableRow>
    );
    ...
    return (
        <>
            <Table className='justify-center'>
                ...
                <TableBody>{raids.map((raid) => content(raid))}</TableBody>
            </Table>
        </>
    );
};
